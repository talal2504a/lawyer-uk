...
<head>
    ...
    <title>LegalConnect | @yield('title', 'Admin Portal')</title>
    ...
    <style>
        ...
        .stat-card-glow:hover {
            box-shadow: 0px 8px 30px rgba(0, 75, 36, 0.08);
...
<div id="dashboard-wrapper" class="flex flex-col md:flex-row dashboard-slide-in">
    ...
    <aside id="sidebar" class="hidden md:block bg-surface-container-low border-r border-outline-variant h-screen w-64 flex flex-col flex-shrink-0 sticky top-0 z-30">
        <div class="px-6 pt-6 pb-4 border-b border-outline-variant">
            ...
            <p class="text-sm text-secondary opacity-70 mt-1 font-['Inter']">Admin Portal</p>
        </div>

        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            <a class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-label-md text-label-md {{ request()->routeIs('admin.dashboard') ? 'active-nav shadow-sm' : 'text-secondary hover:bg-surface-variant hover:text-primary' }}" data-url="{{ route('admin.dashboard') }}">
                <span class="material-symbols-outlined">dashboard</span>
                <span>Dashboard</span>
            </a>
            <a class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-label-md text-label-md {{ request()->routeIs('admin.lawyers') ? 'active-nav shadow-sm' : 'text-secondary hover:bg-surface-variant hover:text-primary' }}" data-url="{{ route('admin.lawyers') }}">
                <span class="material-symbols-outlined">gavel</span>
                <span>Lawyers</span>
            </a>
            <a class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-label-md text-label-md {{ request()->routeIs('admin.customers') ? 'active-nav shadow-sm' : 'text-secondary hover:bg-surface-variant hover:text-primary' }}" data-url="{{ route('admin.customers') }}">
                <span class="material-symbols-outlined">group</span>
                <span>Customers</span>
            </a>
            <a class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-label-md text-label-md {{ request()->routeIs('admin.specializations') ? 'active-nav shadow-sm' : 'text-secondary hover:bg-surface-variant hover:text-primary' }}" data-url="{{ route('admin.specializations') }}">
                <span class="material-symbols-outlined">category</span>
                <span>Specializations</span>
            </a>
            <a class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 font-label-md text-label-md {{ request()->routeIs('admin.appointments') ? 'active-nav shadow-sm' : 'text-secondary hover:bg-surface-variant hover:text-primary' }}" data-url="{{ route('admin.appointments') }}">
    ...
    <main id="main-content" class="flex-1 min-h-screen flex flex-col">
        ...
        <header class="flex justify-between items-center px-gutter w-full h-16 sticky top-0 z-40 bg-surface border-b border-outline-variant">
            <div class="flex items-center gap-4 flex-1">
                <h2 class="font-headline-md text-headline-md font-bold text-primary page-enter" id="page-title-text">@yield('page-title', 'Admin Portal')</h2>
            </div>
            ...
            <span class="material-symbols-outlined text-2xl" id="sidebar-icon">menu</span>
                ...
                <div class="flex items-center gap-3">
                    <div class="text-right max-w-[140px] truncate">
                        <p class="font-label-md text-on-surface">{{ Auth::user()->name ?? 'Admin' }}</p>
                        <p class="text-caption text-secondary">Administrator</p>
                    </div>
        ...
        link.addEventListener('click', function(e) {
            ...
            setTimeout(function() {
                ...
                .then(function(html) {
                    ...
                    var newTitle = doc.querySelector('title')?.textContent || 'Admin Portal';
                    var newPageTitle = doc.querySelector('[id="page-title"]')?.textContent || 'Admin Portal';
